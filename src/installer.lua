local insPath = "/usr/";
local remoteRoot = "https://raw.githubusercontent.com/chorman0773/CCLibraries/master/";
local packageDescriptor = "package";
local tmpDir = "/tmp";
...
local function fromHexChars(str)
  local ret = "";
  for q=1,#str,2 do
    local octet = str:sub(q,q+1);
    ret = ret..string.char(tonumber(octet,16));
  end
  return ret;
end
...
local function downloadFile(line)
  local rpath,lpath,hash;
  rpath, line = nextToken(line);
  lpath, hash = nextToken(line);
  hash = fromHexChars(hash);
  local req = http.get(remoteRoot..rpath);
  local str = req.readAll();
  req.close();
  local chash = SHA.SHA256(str);
  if chash ~= hash then
    return
  end
  local file;
  if lpath[0] == '/' then
    ...
    file = insPath..lpath,"w";
  end
  local fname = file;
  local dname = tmpDir..fs.getDir(fname);
  if not fs.exists(dname) then
    fs.makeDir(dname);
  end
  file = io.open(tmpDir..file);
  file.write(str);
  file.close();
  return fname;
end

local function doFinal(files)
  for i,v in ipairs(files) do
    local dname = fs.getDir(v);
    if not fs.exists(dname) then
      fs.makeDir(dname);
    end
    fs.move(tmpDir..v,v);
  ...
  fs.delete(tmpDir);
  if fs.exists(insPath.."postinstall") then
    os.run(setmetatable({},{__index=_G}),insPath.."postinstall");
  end
end

local function doDescriptor()
  local h = http.get(remoteRoot..packageDescriptor);
  local str;
  local files = {};
  local fid = 1;
  repeat
    str = h.readLine();
    files[fid] = downloadFile(str);
    if not files[fid] then
      fs.delete(tmpDir);
      error("Encountered Error while installing "..str);
    end
  until not str
  return files;
end

local files = doDescriptor();
doFinal(files);
